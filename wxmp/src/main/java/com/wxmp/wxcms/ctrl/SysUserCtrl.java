...
public class SysUserCtrl extends BaseCtrl {
	...
	@Autowired
	private TbEmpService sysUserService;
	@Autowired
	private AccountService accountService;
	@ResponseBody
	@RequestMapping(value = "/login", method = RequestMethod.POST)
	public AjaxResult login(TbEmp user) {
		user.setPwd(MD5Util.getMD5Code(user.getPwd()));
		TbEmp sysUser = this.sysUserService.getSysUser(user);
		if (sysUser == null) {
	...
	public AjaxResult updatepwd(TbEmp user) {
