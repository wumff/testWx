...
public class WxApiCtrl extends BaseCtrl{
	...
	@Autowired
	private TplMsgTextService tplMsgService;
	...
	public @ResponseBody String doGet(HttpServletRequest request,@PathVariable String account) {
		...
		MpAccount mpAccount = getMemAccount();//获取缓存中的唯一账号
		if(mpAccount != null){
	 ...
	 * 批量同步微信模板
	 */
	@RequestMapping(value = "/syncWechatTemplates")
	@ResponseBody
	public AjaxResult syncTplFromWX() throws WxErrorException  {
		MpAccount mpAccount = WxMemoryCacheClient.getMpAccount();//获取缓存中的唯一账号
		if (mpAccount != null) {
			myService.syncTplList(mpAccount);//同时更新数据库
			return AjaxResult.success();
		}
		return AjaxResult.failure();
	}
	...
	@ResponseBody
	public AjaxResult sendTemplateMessage(String tplid,String openIds) throws WxErrorException, BusinessException  {
		MpAccount mpAccount = WxMemoryCacheClient.getMpAccount();//获取缓存中的唯一账号
		TplMsgText tpl=tplMsgService.getById(tplid);
		TemplateMessage tplMsg = new TemplateMessage();

		String[] openIdArray = StringUtils.split(openIds, ",");
		String[] paramArray=StringUtils.split(tpl.getWxTplParam(), ";");
		Map<String, String> dataMap = new HashMap<String, String>();
		for(String param:paramArray){
			String[] kv = StringUtils.split(param, ":");
			if(kv==null||kv.length!=2){
				throw new BusinessException("模板消息参数设置格式错误");
			}
		...
		for (String openId : openIdArray) {
			...
			tplMsg.setTemplateId(tpl.getTplId());
...
//				}
	...
	private Account getMemAccount() {
		Account account = accountService.getByAccount(WxMemoryCacheClient.getAccount());
		if (null==account) {
