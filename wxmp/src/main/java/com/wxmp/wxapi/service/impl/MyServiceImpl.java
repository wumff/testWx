...
public class MyServiceImpl implements MyService {
  ...
  //同步模板列表
    public boolean syncTplList(MpAccount mpAccount) throws WxErrorException {
        String url=null;
        try {
            url = WxApi.getTemplateListURL(WxApiClient.getAccessToken(mpAccount));
        } catch (WxErrorException e) {
            e.printStackTrace();
        }
        logger.info("同步微信模板列表如下:"+url);
        JSONObject jsonObject = WxApi.httpsRequest(url, HttpMethod.GET, null);
        logger.info("同步微信模板列表如下:"+jsonObject.toString());
        if(jsonObject.containsKey("errcode")){
            return false;
        }
        JSONArray arr = jsonObject.getJSONArray("template_list");//获取jsonArray对象
        String js=JSONObject.toJSONString(arr);//将array数组转换成字符串
        List<WXTemplate> tpls=JSONObject.parseArray(js, WXTemplate.class);//把字符串转换成集合
        
        tpldao.syncTpls(tpls);
        return true;
    }
}
